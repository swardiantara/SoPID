...
for embedding in "${word_embeds[@]}"; do
    for seed in "${seeds[@]}"; do
        for freeze in "${freezes[@]}"; do
            if [ "$freeze" = true ]; then
                python train_message.py --embedding "$embedding" --seed "$seed" --feature_col message --n_epochs 100 --freeze_embedding 
            else
                python train_message.py --embedding "$embedding" --seed "$seed" --feature_col message --n_epochs 100
            fi
        done
    done
done